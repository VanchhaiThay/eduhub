...
class _CourseTabState extends State<CourseTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: AnimatedSwitcher(
        ...
        child: _selectedIndex == 0 ? _buildCourseContent(isDark) : _buildLibraryContent(isDark),
      ),
    );
  }
  ...
  Widget _drawerItem(IconData icon, String title, int index, bool isDark) {
    ...
    return ListTile(
      ...
      title: Text(title, style: TextStyle(fontWeight: selected ? FontWeight.bold : FontWeight.normal, color: selected ? Colors.teal : null)),
      onTap: () {
  ...
  Widget _buildSubjectCard(String title, IconData icon, Color color, bool isDark) {
    return Column(
      children: [
        Expanded(
          child: Container(
      ...
      itemBuilder: (context, index) {
        return Container(
          ...
          child: ListTile(
            ...
            title: Text("Reference Material ${index + 1}", style: const TextStyle(fontWeight: FontWeight.w700)),
            subtitle: const Text("Grade 12 • Chemistry • 5.2 MB", style: TextStyle(fontSize: 12)),
            trailing: IconButton(
              icon: const Icon(Icons.download_for_offline_rounded, color: Colors.teal),
              onPressed: () {},
            ),
          ),
        );
